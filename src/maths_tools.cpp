...
#include <Rcpp.h>
 ...
 //'  
 //' @param seed description of the seed
 //' @export
 // [[Rcpp::export]]
 ...
 //' myrv
 //' 
 //' trictly runif(1); get random from R for reproductibility
 //' 
 //' @param None
 // [[Rcpp::export]]
 double myrv() {
 ...
 NumericVector generate_permutation3(int permutation_size, int total_size){
   ...
   for (int i = 0; i < permutation_size; i++){
     ...
     //Rcout << "OK here it's done..."<<v(alea) << std::endl;
     result(i) = v(alea);
 ...
 //' proba_matrix_mult
 //' 
 //' return the special matrix product between two probability matrix. (obs)
 //'
