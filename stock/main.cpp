...
Rcpp::List optgenam3 (Eigen::SparseMatrix<double> currentPresenceMatrix,
  ...
  // Count for number of planting possibilities in space (nbpp), in each period.
     ...
     //std::cout << npm <<std::endl;
      ...
      if((evaluate(threshold)+evaluate(threshold+1))>=confidence)
      {
        ...
        while(evaluate(threshold+1)>=confidence){
          ...
          int index_eliminated = optimise_remove_site(viablesValues2,threshold,confidence,viablesTriplets,population,j,nbtoplant,npm);
        ...
        if(evaluate(threshold)>=confidence){
          optimise_add_site(viablesValues2,threshold,confidence,viablesTriplets,population,j,nbtoplant,npm);
          evaluation(j,0)=1;
        }
        else{
          evaluation(j,0)=0;
